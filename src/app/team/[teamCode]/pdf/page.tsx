...
export default async function CouponPDFPage({ params }: Props) {
    ...
    // Only team's own google account OR committee can view
    const isTeamLead = team.teamLead?.email === session.user.email;
    if (!isTeamLead) {
        // Allow if they have an event role (committee)
    ...
    // Group coupons by memberId, then by meal type
    const memberCouponMap: Record<string, Record<string, { code: string; qrUrl: string }>> = {};

    for (const coupon of coupons) {
        if (!memberCouponMap[coupon.memberId]) {
            memberCouponMap[coupon.memberId] = {};
        }
        const qrUrl = await fetchQRDataUrl(coupon.couponCode);
        memberCouponMap[coupon.memberId][coupon.type] = {
            ...
            <head>
                ...
                <style>{`
                    * { box-sizing: border-box; margin: 0; padding: 0; }
                    ...
                    .coupon-card .event-name { font-size: 11px; color: #9ca3af; margin-top: 6px; }
                    .no-coupon {
                        background: #f9fafb;
                                ...
                                {mealSlots.map((slot) => {
                                    ...
                                    return (
                                        <div key={slot} className="coupon-card">
                                            ...
                                            <img src={c.qrUrl} alt={c.code} />
                                            <div className="code">{c.code}</div>
                                            <div className="event-name">{member.name}</div>
                                        </div>
                                    );
                                })}
                            </div>
